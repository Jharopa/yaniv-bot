...
class Basic(commands.Cog, name='Basic Commands'):
    ...
    @website.error
    async def website_error(self, ctx: commands.Context, error: commands.CommandError) -> None:
    ...
    @qoute.error
    async def qoute_error(self, ctx: commands.Context, error: commands.CommandError) -> None:
        if isinstance(error, commands.CheckFailure):
            return
